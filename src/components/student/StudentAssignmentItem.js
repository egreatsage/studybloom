...
import { FaChevronDown, FaLink, FaCheckCircle, FaHourglassHalf, FaClock, FaFileAlt, FaStar } from 'react-icons/fa';
...
const StudentAssignmentItem = ({ assignment, studentId, onSubmission }) => {
  ...
  // The API now only returns the student's own submission in the array, so we can safely get the first element.
  const mySubmission = assignment.submissions?.[0];
  const isPastDue = new Date(assignment.dueDate) < new Date();

  // THIS IS THE FIX: Check if the grade property is a number (this includes 0).
  const isGraded = mySubmission && typeof mySubmission.grade === 'number';

  const getStatus = () => {
    if (isGraded) {
      const grade = mySubmission.grade;
      const gradeColor = grade >= 90 ? 'text-green-600' : grade >= 70 ? 'text-blue-600' : grade >= 50 ? 'text-yellow-600' : 'text-red-600';
      const bgColor = grade >= 90 ? 'bg-green-100 text-green-800' : grade >= 70 ? 'bg-blue-100 text-blue-800' : grade >= 50 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800';
      
      return (
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1">
            <FaStar className={`w-4 h-4 ${gradeColor}`} />
            <span className={`font-bold text-xl ${gradeColor}`}>{grade}/100</span>
          </div>
    ...
    if (mySubmission) {
      return (
        <span className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium bg-blue-100 text-blue-800 rounded-full shadow-sm">
    ...
    if (isPastDue) {
      return (
        <span className="px-3 py-1.5 text-xs font-medium bg-red-100 text-red-700 rounded-full shadow-sm">
          Past Due
        </span>
      );
    }
    return (
      <span className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium bg-amber-100 text-amber-800 rounded-full shadow-sm">
